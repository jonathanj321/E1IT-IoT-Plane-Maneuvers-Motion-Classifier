...
void TrainOrientation_SM_Translation(ANN *net) {
	...
	for (k = 0; k < num_train_data_cycles; k++) {
		for (i = 0; i < 6; i++) {
			...
			case 0:
				printf("\nTrain Descending Right Turn into Level Off\n");
				break;
			case 1:
				printf("\n\nTrain Ascending Left Turn into Level Off\n");
				break;
			case 2:
				printf("\n\nTrain S Turn into Climb\n");
				break;
			case 3:
				printf("\n\nTrain Steep Turn\n");
				break;
			case 4:
				printf("\n\nTrain Right Traffic Pattern from Takeoff\n");
				break;
			case 5:
				printf("\n\nTrain Left Traffic Pattern from Landing\n");
				break;
			}
...
void Test_ANN_SM_Translation(ANN *net, int32_t *loc) {
	...
	const char* motion_names[6] = {
	    "Descending Right Turn into Level Off",
	    "Ascending Left Turn into Level Off",
	    "S Turn into Climb",
	    "Steep Turn",
	    "Right Traffic Pattern from Takeoff",
	    "Left Traffic Pattern from Landing"
	};

	int motion_order[6] = {0, 1, 2, 3, 4, 5};

	for (i = 5; i > 0; i--) {
		int j = rand() % (i + 1);
		int temp = motion_order[i];
		motion_order[i] = motion_order[j];
		motion_order[j] = temp;
	}

	for (int idx = 0; idx < 6; idx++) {
		int expected_motion = motion_order[idx];
		switch (expected_motion) {
					case 0:
						printf("\n\rPlease perform a Descending Right Turn into Level Off\n");
						break;
					case 1:
						printf("\n\rPlease perform a Ascending Left Turn into Level Off\n");
						break;
					case 2:
						printf("\n\rPlease perform an S Turn into Climb\n");
						break;
					case 3:
						printf("\n\rPlease perform a Steep Turn\n");
						break;
					case 4:
						printf("\n\rPlease perform a Right Traffic Pattern from Takeoff\n");
						break;
					case 5:
						printf("\n\rPlease perform a Left Traffic Pattern from Landing\n");
						break;
					case -1:
	...
	if (*loc == expected_motion) {
	    printf("\nCorrectly performed: %s\n", motion_names[*loc]);
		}
	else if (*loc == -1) {
		printf("\nClassification Failed, Please Retry.\n");
	}
	else {
	    printf("\nMismatch:\n Expected: %s\n Got: %s\n",
	           motion_names[expected_motion],
	           (*loc >= 0 && *loc < 6) ? motion_names[*loc] : "INVALID");
	}
